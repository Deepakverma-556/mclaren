    ...
    return (
        <section className='bg-blue'>
            <div className='max-w-[1215px] px-4 mx-auto max-lg:pl-8 max-md:px-4'>
                <div className='flex md:items-end items-center justify-between pt-11 relative pb-[15px] max-md:py-6 max-sm:py-3'>
                    ...
                    <ul className='flex items-center gap-6 max-md:hidden'>
                        {HEADER_LINK.map((obj, i) => (
                            <li key={i}><NavLink to={obj.link} className='text-white text-[13px] font-notoSans leading-[15px] relative flex after:absolute after:w-0 after:transition-all after:duration-300 after:hover:w-full after:bottom-0 after:h-[0.2px] after:bg-white '>{obj.title}</NavLink></li>
                        ))}
                    </ul>
                    <form action="" className='max-w-[257px] border border-white rounded-[5px] flex items-center justify-between overflow-hidden max-md:hidden pr-3 absolute right-0'>
                    ...
                    <div className={`w-full h-full transition-all duration-500 left-0 lg:-top-full z-20 fixed flex flex-col bg-blue justify-center items-center ${open ? "top-0 left-0" : "-top-full"
                        ...
                        <ul className='flex items-center flex-col gap-6 md:hidden'>
                            {HEADER_LINK.map((obj, i) => (
                                <li key={i}><NavLink to={obj.link} className='text-white text-[13px] font-notoSans leading-[15px]'>{obj.title}</NavLink></li>
                            ))}
